...
import com.varahunter.nebulithic.block.Sclerotis261SnowBlock;
import com.varahunter.nebulithic.block.Sclerotis261RegolithBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class EczemicSnowyHillyBarrensBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:eczemic_snowy_hilly_barrens")
	public static final CustomBiome biome = null;
	public EczemicSnowyHillyBarrensBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 139);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.parent("nebulithic_ascension_rewritten:eczemic_hilly_barrens")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Sclerotis261SnowBlock.block.getDefaultState(),
							Sclerotis261RegolithBlock.block.getDefaultState(), Sclerotis261RegolithBlock.block.getDefaultState())));
			setRegistryName("eczemic_snowy_hilly_barrens");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Sclerotis261SnowBlock.block.getDefaultState().getBlock()
							|| ground == Sclerotis261RegolithBlock.block.getDefaultState().getBlock())
							&& (ground2 == Sclerotis261SnowBlock.block.getDefaultState().getBlock()
									|| ground2 == Sclerotis261RegolithBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Sclerotis261RegolithBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Sclerotis261SnowBlock.block.getDefaultState().getBlock()
					|| blockType == Sclerotis261RegolithBlock.block.getDefaultState().getBlock();
		}
