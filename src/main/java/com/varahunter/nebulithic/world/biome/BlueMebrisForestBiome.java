...
import com.varahunter.nebulithic.block.MebrisDirtBlock;
import com.varahunter.nebulithic.block.BlueMebrisGrassBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class BlueMebrisForestBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:blue_mebris_forest")
	public static final CustomBiome biome = null;
	public BlueMebrisForestBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 77);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(BlueMebrisGrassBlock.block.getDefaultState(),
							MebrisDirtBlock.block.getDefaultState(), MebrisDirtBlock.block.getDefaultState())));
			setRegistryName("blue_mebris_forest");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlueMebrisGrassBlock.block.getDefaultState().getBlock()
							|| ground == MebrisDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == BlueMebrisGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == MebrisDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MebrisDirtBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlueMebrisGrassBlock.block.getDefaultState().getBlock()
					|| blockType == MebrisDirtBlock.block.getDefaultState().getBlock();
		}
