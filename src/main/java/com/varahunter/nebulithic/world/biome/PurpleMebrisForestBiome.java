...
import com.varahunter.nebulithic.block.SmoothMebrisDirtBlock;
import com.varahunter.nebulithic.block.PurpleMebrisGrassBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class PurpleMebrisForestBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:purple_mebris_forest")
	public static final CustomBiome biome = null;
	public PurpleMebrisForestBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 56);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(PurpleMebrisGrassBlock.block.getDefaultState(),
							SmoothMebrisDirtBlock.block.getDefaultState(), SmoothMebrisDirtBlock.block.getDefaultState())));
			setRegistryName("purple_mebris_forest");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == PurpleMebrisGrassBlock.block.getDefaultState().getBlock()
							|| ground == SmoothMebrisDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == PurpleMebrisGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == SmoothMebrisDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), SmoothMebrisDirtBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == PurpleMebrisGrassBlock.block.getDefaultState().getBlock()
					|| blockType == SmoothMebrisDirtBlock.block.getDefaultState().getBlock();
		}
