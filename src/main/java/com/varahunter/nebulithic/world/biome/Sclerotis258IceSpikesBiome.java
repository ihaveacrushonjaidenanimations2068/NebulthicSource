...
import com.varahunter.nebulithic.block.Sclerotis258SnowBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class Sclerotis258IceSpikesBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:sclerotis_258_ice_spikes")
	public static final CustomBiome biome = null;
	public Sclerotis258IceSpikesBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 183);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.parent("nebulithic_ascension_rewritten:snowy_hilly_barrens")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Sclerotis258SnowBlock.block.getDefaultState(),
							Sclerotis258SurfaceRockBlock.block.getDefaultState(), Sclerotis258SurfaceRockBlock.block.getDefaultState())));
			setRegistryName("sclerotis_258_ice_spikes");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Sclerotis258SnowBlock.block.getDefaultState().getBlock()
							|| ground == Sclerotis258SurfaceRockBlock.block.getDefaultState().getBlock())
							&& (ground2 == Sclerotis258SnowBlock.block.getDefaultState().getBlock()
									|| ground2 == Sclerotis258SurfaceRockBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Sclerotis258SurfaceRockBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Sclerotis258SnowBlock.block.getDefaultState().getBlock()
					|| blockType == Sclerotis258SurfaceRockBlock.block.getDefaultState().getBlock();
		}
