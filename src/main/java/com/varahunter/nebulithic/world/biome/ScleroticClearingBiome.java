...
import com.varahunter.nebulithic.block.ScleroticGrassBlock;
import com.varahunter.nebulithic.block.ScleroticDirtBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class ScleroticClearingBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:sclerotic_clearing")
	public static final CustomBiome biome = null;
	public ScleroticClearingBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 100);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(ScleroticGrassBlock.block.getDefaultState(),
							ScleroticDirtBlock.block.getDefaultState(), ScleroticDirtBlock.block.getDefaultState())));
			setRegistryName("sclerotic_clearing");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ScleroticGrassBlock.block.getDefaultState().getBlock()
							|| ground == ScleroticDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == ScleroticGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == ScleroticDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), ScleroticDirtBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == ScleroticGrassBlock.block.getDefaultState().getBlock()
					|| blockType == ScleroticDirtBlock.block.getDefaultState().getBlock();
		}
