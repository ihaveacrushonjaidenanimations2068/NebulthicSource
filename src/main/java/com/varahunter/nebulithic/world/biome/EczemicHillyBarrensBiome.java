...
public class EczemicHillyBarrensBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:eczemic_hilly_barrens")
	public static final CustomBiome biome = null;
	public EczemicHillyBarrensBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 138);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Sclerotis261RegolithBlock.block.getDefaultState(),
							Sclerotis261RegolithBlock.block.getDefaultState(), Sclerotis261RegolithBlock.block.getDefaultState())));
			setRegistryName("eczemic_hilly_barrens");
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Sclerotis261RegolithBlock.block.getDefaultState().getBlock()
