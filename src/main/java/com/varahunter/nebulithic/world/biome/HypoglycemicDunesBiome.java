...
import com.varahunter.nebulithic.entity.HypotarantulaEntity;
import com.varahunter.nebulithic.block.HypoglycemicSandstoneBlock;
import com.varahunter.nebulithic.block.HypoglycemicSandBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class HypoglycemicDunesBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:hypoglycemic_dunes")
	public static final CustomBiome biome = null;
	public HypoglycemicDunesBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 134);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(HypoglycemicSandBlock.block.getDefaultState(),
							HypoglycemicSandstoneBlock.block.getDefaultState(), HypoglycemicSandstoneBlock.block.getDefaultState())));
			setRegistryName("hypoglycemic_dunes");
			...
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(HypotarantulaEntity.entity, 20, 4, 4));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == HypoglycemicSandBlock.block.getDefaultState().getBlock()
							|| ground == HypoglycemicSandstoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == HypoglycemicSandBlock.block.getDefaultState().getBlock()
									|| ground2 == HypoglycemicSandstoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), HypoglycemicSandstoneBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == HypoglycemicSandBlock.block.getDefaultState().getBlock()
					|| blockType == HypoglycemicSandstoneBlock.block.getDefaultState().getBlock();
		}
