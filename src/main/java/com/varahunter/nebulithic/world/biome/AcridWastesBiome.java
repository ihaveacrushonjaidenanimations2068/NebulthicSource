...
import com.varahunter.nebulithic.block.PlumesandBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class AcridWastesBiome extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:acrid_wastes")
	public static final CustomBiome biome = null;
	public AcridWastesBiome(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 54);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(PlumesandBlock.block.getDefaultState(),
							PlumesandBlock.block.getDefaultState(), PlumesandBlock.block.getDefaultState())));
			setRegistryName("acrid_wastes");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == PlumesandBlock.block.getDefaultState().getBlock() || ground == PlumesandBlock.block.getDefaultState().getBlock())
							&& (ground2 == PlumesandBlock.block.getDefaultState().getBlock()
									|| ground2 == PlumesandBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), PlumesandBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == PlumesandBlock.block.getDefaultState().getBlock()
					|| blockType == PlumesandBlock.block.getDefaultState().getBlock();
		}
