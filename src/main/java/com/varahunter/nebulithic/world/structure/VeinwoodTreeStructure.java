...
public class VeinwoodTreeStructure extends NebulithicAscensionRewrittenModElements.ModElement {
	public VeinwoodTreeStructure(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 30);
	}
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 10000) {
					...
					for (int a = 0; a < count; a++) {
						...
						if (blockAt.getBlock() == AshBlock.block.getDefaultState().getBlock())
							blockCriteria = true;
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("nebulithic_ascension_rewritten", "veinwood_tree"));
						if (template == null)
							return false;
