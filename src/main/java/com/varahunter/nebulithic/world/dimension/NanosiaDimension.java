...
import com.varahunter.nebulithic.block.NanostoneBlock;
import com.varahunter.nebulithic.NebulithicAscensionRewrittenModElements;

import com.google.common.collect.ImmutableSet;

@NebulithicAscensionRewrittenModElements.ModElement.Tag
public class NanosiaDimension extends NebulithicAscensionRewrittenModElements.ModElement {
	@ObjectHolder("nebulithic_ascension_rewritten:nanosia")
	public static final ModDimension dimension = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public NanosiaDimension(NebulithicAscensionRewrittenModElements instance) {
		super(instance, 257);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("nanosia"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("nebulithic_ascension_rewritten:nanosia")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("nebulithic_ascension_rewritten:nanosia"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("nebulithic_ascension_rewritten:nanosia"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("nebulithic_ascension_rewritten:untouched_fields")),};
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return NanostoneBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(NanostoneBlock.block.getDefaultState().getBlock(),
